...
| `source.ip` | Source IP address | `src_ip`, `client.ip`, `remote_addr`, `http.request.headers.x-forwarded-for` |
...
| Not escaping special chars | Query errors | Escape: `\/uploads\/` or use quotes |
...
1. Check field names exist: `| fields process.*, http.*, user.*`
2. Verify data is ingested: `event.dataset: "apache.access" | stats count`
3. Check time range: Remove time filter and test
4. Verify index pattern matches your data

**If query is slow:**
1. Add time range filters first
2. Use specific index patterns
3. Filter by dataset early: `event.dataset: "process" AND ...`
4. Avoid wildcards at the start of strings
